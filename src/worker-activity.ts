...
export type WorkerState = "spawning" | "running" | "idle" | "completed" | "failed" | "aborted" | "paused";
...
const STATES = new Set(["spawning", "running", "idle", "completed", "failed", "aborted", "paused"]);
...
export function visibleWorkers(cards: WorkerCard[]): WorkerCard[] {
  return cards.filter(card => isWorking(card) || needsAttention(card))
